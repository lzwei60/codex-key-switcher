...
`v1.0.0` 是当前稳定发布版本。Release tag 应使用 `v1.0.0`，应用内版本号和安装包版本号应使用 `1.0.0`。

必须上传的 Release 资源：

- `Codex-Key-Switcher-1.0.0-arm64.dmg`，用于 macOS Apple Silicon。
- `Codex-Key-Switcher-1.0.0-x64.dmg`，用于 macOS Intel。
- `Codex-Key-Switcher-Setup-1.0.0-x64.exe`，用于 Windows x64。
